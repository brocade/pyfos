...
:mod:`cfgadd` - PyFOS util for specific Zoning use case.
***********************************************************************************
The :mod:`cfgadd` provides for specific Zoning use case.

This module is a standalone script and API that can be used to add new
member to existing cfg(s).
...
def cfgadd(session, cfgs):
    """Add to an existing cfg(s) additional member(s)
    ...
    Example usage of the method::
        ...
        result = cfgadd(session, cfgs)

    :param session: session returned by login
    :param cfgs: an array of cfg and new members
    :rtype: dictionary of return status matching rest response

    *use cases*

        1. add a new member to an existing cfg
...
def __cfgadd(session, name, members):
    cfgs = [
            {"cfg-name": name, "member-zone": {"zone-name": members}}
           ]
    return cfgadd(session, cfgs)


def usage():
    print("usage:")
    print('cfgadd.py -i <ipaddr> -n <name> -m <members>')
...
def main(argv):
    ...
    brcd_zone_util.zone_name_members_func(session, inputs, usage, __cfgadd)
