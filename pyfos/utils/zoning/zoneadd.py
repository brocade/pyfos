...
:mod:`zoneadd` - PyFOS util for specific Zoning use case.
***********************************************************************************
The :mod:`zoneadd` provides for specific Zoning use case.

This module is a standalone script and API that can be used to add new
member to existing Zone(s).
...
def zoneadd(session, zones):
    """Add to an existing Zone(s) additional member(s)
    ...
    *use cases*
        ...
        1. add a new member to an existing Zone(s)
...
def __zoneadd(session, name, members):
    zones = [
                {
                    "zone-name": name,
    ...
    return zoneadd(session, zones)


def usage():
    print("usage:")
    print('zoneadd.py -i <ipaddr> -n <name> -m <members>')
...
def main(argv):
    ...
    brcd_zone_util.zone_name_members_func(session, inputs, usage, __zoneadd)
