...
:mod:`switchipinterfacedelete` - PyFOS util for creating a switch IP address.
********************************************************************************
The :mod:`switchipinterfacedelete` provides IP Address creation functionality.
...
.. function:: switchipinterfacedelete.delete_extension_ip(session, name, dp,\
ip, prefix, vlan=None, mtu=None)
    
    *Delete extension IP Address* 

        Example usage of the method::
        
            ret = switchipinterfacedelete.delete_extension_ip(session, name,\
            dp, ip, prefix, vlan=None, mtu=None)
            print (ret)
        ...
        Details::
            ...
            result = switchipinterfacedelete._delete_extension_ip(session, IP)
        ...
        *use cases*
        ...
        1. Delete an extension IP Interface
"""
...
def parse_ip_addr(user_command, inputs,  value_dict):
	try:
		opts, args = getopt.getopt(user_command,"i:n:d:s:v:m:p:f:",
                ["switch=","name=","dp-id=","ip-address=",
                 "vlan-id=","mtu-size=", "ip-prefix-length=","fid=", ])
...
def _delete_extension_ip(session, value_dict):
	ipobject = extension_ipaddress(value_dict)
...
def delete_extension_ip(session, name, dp, ip, prefix, vlan=None, mtu=None):
	...
	if mtu != None:
		value_dict.update({'mtu-size': arg})
	result = _delete_extension_ip(value_dict)
  

def usage():
	print ("usage:")
	print ('switchipinterfacedelete.py -i <ipaddr> -n <name> ' +\
               '-d <dp-id> -s <ip-address> -p <ip-prefix-length> ' +\
               '[-v<vlan-id> -m <mtu-size>]')


def main(argv):
	#myinput=str("-i 10.17.3.70  -n 4/17 -d 0 -s 134.10.10.1 -p 24")
	#argv = myinput.split()
	value_dict = dict()
	inputs = dict()
	...
	if "dp-id" not in value_dict:
		...
		usage()
		sys.exit()        
	result = _delete_extension_ip(session, value_dict)
