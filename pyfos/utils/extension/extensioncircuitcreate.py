...
:mod:`extensioncircuitcreate` - PyFOS util for creating a circuit object.
***********************************************************************************
The :mod:`extensioncircuitcreate` provides circuit creation functionality.
...
.. function:: extensioncircuitcreate.create_extension_circuit(session, name, cid, local,\
remote, minB = 500000, maxB=50000)
    ...
    *Create extension circuit* 
        ...
        Example usage of the method::
                ...
                ret = extensioncircuitcreate.create_extension_circuit(session, name, cid,\
                local, remote, minB = 500000, maxB=50000)
                print (ret)
        ...
        Details::
            ...
            result = extensioncircuitcreate._create_extension_circuit(session, circuit)
        ...
        *use cases*
        ...
        1. create a new circuit to an existing tunnel
...
def _create_extension_circuit(session, value_dict):
...
def create_extension_circuit(session, name, cid, local, remote,
        ...
        value_dict = {'name': name, 'circuit-id': cid,
                      ...
                      'minimum-communication-rate':minB,
                      'maximum-communication-rate': maxB }
        result = _create_extension_circuit(value_dict)
        return result
  

def usage():
	print ("usage:")
	print ('extensioncircuitcreate.py -i <ipaddr> -n <name> -a <admin-enabled> ' +\
...
def main(argv):
	#myinput=str("-i 10.17.3.70  -n 4/19 -c 0 -S 134.10.10.1 -D 154.10.10.1" +\
        #            " -b 50000 -B 50000")
	#argv = myinput.split()
	value_dict = dict()
	inputs = dict()
	...
	result = _create_extension_circuit(session, value_dict)
