...
# extension_ipsec_policy_create.py(pyGen v1.0.0)


"""

:mod:`extension_ipsec_policy_create` - PyFOS util to create for\
 extension_ipsec_policy
******************************************************************************\
*******************************************************************************
The:mod:`extension_ipsec_policy_create` PyFOS util to create for\
 extension_ipsec_policy
...
session, profile_name, policy_name, authentication_data)

    *Create extension_ipsec_policy*

    Example Usage of the Method::

            ret =\
 extension_ipsec_policy_create.create_extension_ipsec_policy(session,\
 profile_name, policy_name, authentication_data)
            print(ret)
    ...
    Details::
        ...
        ret = _create_extension_ipsec_policy(session,\
...
def _create_extension_ipsec_policy(session, extension_ipsec_policyObj):
    return extension_ipsec_policyObj.post(session)


def create_extension_ipsec_policy(session, profile_name=None,
                                  policy_name=None, authentication_data=None):
    extension_ipsec_policyObj = extension_ipsec_policy()
    extension_ipsec_policyObj.set_profile_name(profile_name)
    extension_ipsec_policyObj.set_policy_name(policy_name)
    extension_ipsec_policyObj.set_authentication_data(authentication_data)
    return _create_extension_ipsec_policy(session, extension_ipsec_policyObj)
...
def main(argv):
    filters = ["profile_name", "policy_name", "authentication_data"]
    inputs = brcd_util.parse(argv, extension_ipsec_policy, filters, validate)
    session = brcd_util.getsession(inputs)
    result = _create_extension_ipsec_policy(session, inputs['utilobject'])
