...
.. function:: switchiproutemodify.modify_extension_iproute(session, name,\
dp, ip, prefix, gateway)
    ...
    *Modify an extension IP route* 
        ...
        Example usage of the method::
            ...
            ret = switchiproutemodify.modify_extension_iproute(session, name,\
            dp, ip, prefix, gateway)
            print (ret)
...
def _modify_extension_iproute(session, value_dict):
	iprouteobject = extension_iproute()
	iprouteobject.load(value_dict, 1)
	result = iprouteobject.patch(session)
	return result

def modify_extension_iproute(session, name, dp, ip, gateway):
...
def usage():
    print ("usage:")
    print ('switchiproutemodify.py -i <ipaddr> -n <name> -d <dp-id> ' +\
