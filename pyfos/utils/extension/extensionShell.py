...
from pyfos.pyfos_extension_tunnel import extension_tunnel_statistics
from pyfos.pyfos_extension_circuit import extension_circuit
from pyfos.pyfos_extension_circuit import extension_circuit_statistics
from pyfos.pyfos_rest_util import rest_obj_type
...
def usage(myobject = None, cmd = 'show'):
	if myobject:
		if isinstance(myobject, extension_ipaddress):
			if cmd == 'show':
				print (show_ipif_str)
			if cmd == 'cfg':
				print (cfg_ipif_str)
		if isinstance(myobject, extension_iproute):
			if cmd == 'show':
...
def parse_circuit(user_command, myobject, value_dict): 
	try:
		opts, args = getopt.getopt(user_command,"b:B:S:D:",["min-comm-rate=","max-comm-rate=","local-ip", "remote-ip"])
...
def parse_port(arg, myobject, value_dict):
        ...
        if (pattern_verify) :
                ...
                if len(port) >0:
                        name  = port[0]
                        if isinstance(myobject, extension_ipaddress) or isinstance(myobject, extension_iproute) :
...
def parse_ipaddress(arg, myobject, value_dict):
	...
	if isinstance(myobject, extension_ipaddress) or isinstance(myobject, extension_iproute) :
		print ("Incorrect ip-address/ip-prefix-length value specified \"" + arg + "\"")
		return 1
	else:
		return 0
...
def parse_ipgateway(arg, myobject, value_dict):
	...
	if isinstance(myobject, extension_ipaddress) or isinstance(myobject, extension_iproute) :
		print ("Incorrect ip-gateway value specified \"" + arg + "\"")
		return 1
	else:
		return 0
...
def parse_cid(arg, myobject, value_dict):
	...
	if pattern_verify:
		if isinstance(myobject, extension_circuit) or isinstance(myobject, extension_circuit_statistics):
			print  (arg), pattern_verify
			value_dict.update({'circuit-id': arg})
			return 0
	if isinstance(myobject, extension_circuit) :
		print ("Incorrect cid value specified \"" + arg + "\"")
		return 1
	else:
		return 0
...
def handler(user_command):
	...
	while i < argc  and i <= 4:
		...
		if i == 1 and argv[i] == 'ipif':
			myobject = extension_ipaddress()
		if i == 1 and argv[i] == 'iproute':
			myobject = extension_iproute()
...
def main(argv):
	...
	if 'login-error' in session.get('credential').keys():
		print ("login failed because", session.get('credential')['login-error'])
		sys.exit()
