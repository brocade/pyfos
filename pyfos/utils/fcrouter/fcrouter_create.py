...
# fcrouter_create.py(pyGen v1.0.0)

"""

:mod:`fcrouter_create` - PyFOS util to add FCR attributes
*******************************************************************************
The :mod:`fcrouter_create` PyFOS util to add FCR attributes


Generic FCR configuration parameters.

fcrouter_create : usage
...
.. function:: fcrouter_create.create_routing_configuration(session,\
lsan_enforce_tags_tag, shortest_ifl, lsan_speed_tag, backbone_fabric_id,\
maximum_lsan_count)

    *Create routing_configuration*

        Example Usage of the Method::

            ret = fcrouter_create.create_routing_configuration(session,\
...
def _create_routing_configuration(session, routing_configurationObj):
    return routing_configurationObj.post(session)


def create_routing_configuration(session, lsan_enforce_tags_tag=None,
                                 lsan_speed_tag=None):
    routing_configurationObj = routing_configuration()
    routing_configurationObj.set_lsan_enforce_tags_tag(lsan_enforce_tags_tag)
    routing_configurationObj.set_lsan_speed_tag(lsan_speed_tag)
    return _create_routing_configuration(session, routing_configurationObj)
...
def main(argv):
    ...
    result = _create_routing_configuration(session, inputs['utilobject'])
