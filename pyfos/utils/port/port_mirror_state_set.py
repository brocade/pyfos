...
:mod:`port_mirror_state_set` - PyFOS util for specific port op use case.
***********************************************************************************
The :mod:`port_mirror_state_set` provides for specific port op use case.
...
* Util scripts options:
    ...
    -m,--mirror_enabled=MIRROR_ENABLED \
                                    Set "mirror-port-enabled" <0|1>.
...
def validate(fcObject):
    if fcObject.peek_name() is None or \
       fcObject.peek_mirror_port_enabled() is None:
        return 1
    return 0


def main(argv):
    filters = ["name", "mirror_port_enabled"]
