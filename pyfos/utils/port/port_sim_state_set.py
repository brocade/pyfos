...
:mod:`port_sim_state_set` - PyFOS util for specific port op use case.
***********************************************************************************
The :mod:`port_sim_state_set` provides for a specific port op use case.
...
* Util script options:
    ...
    --sim_enabled=SIM_ENABLED           Set "sim-port-enabled" <0|1>
...
def validate(fcObject):
    if fcObject.peek_name() is None or \
       fcObject.peek_sim_port_enabled() is None:
        return 1
    return 0


def main(argv):
    filters = ["name", "sim_port_enabled"]
