...
This module is a stand-alone script that can be used to set/reset  \
fault-delay config for a port.
...
* Util script options:
    ...
    --fault_delay_enabled=FAULT_DELAY_ENABLED \
                                   Set "falut_delay-port-enabled" <0|1>
...
def validate(fcObject):
    if fcObject.peek_name() is None or \
       fcObject.peek_fault_delay_enabled() is None:
        return 1
    return 0


def main(argv):
    filters = ["name", "fault_delay_enabled"]
