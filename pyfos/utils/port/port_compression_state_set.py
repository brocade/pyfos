...
:mod:`port_compression_state_set.py` - PyFOS util for specific port op use case
*******************************************************************************
The :mod:`port_compression_state_set.py` provides for specific port op use case

This module is a stand-alone script that can be used to enable/disable
compression configuration of a port.
...
def validate(fcObject):
    if fcObject.peek_name() is None or \
       fcObject.peek_compression_configured() is None:
        return 1
    return 0


def main(argv):
    filters = ["name", "compression_configured"]
