...
:mod:`port_npiv_state_set` - PyFOS util for specific port op use case.
***********************************************************************************
The :mod:`port_npiv_state_set` provides for a specific port op use case.
...
* Util script options:
    ...
    --npiv_enabled=NPIV_ENABLED           Set "npiv-enabled" <0|1>
...
def validate(fcObject):
    if fcObject.peek_name() is None or \
       fcObject.peek_npiv_enabled() is None:
        return 1
    return 0


def main(argv):
    filters = ["name", "npiv_enabled"]
