...
:mod:`pyfos_switch` - PyFOS module to provide rest support for FC switch.
*********************************************************************************************************
The :mod:`pyfos_switch` provides a REST support for FC switch.
...
class fibrechannel_switch(pyfos_rest_util.rest_object):
    ...
    *Attribute methods*
    ...
    def get(session, name=None):
        if name is None:
            return pyfos_rest_util.get_all(session, fibrechannel_switch)
        else:
            obj = fibrechannel_switch()
            obj.set_name(name)
            showobj = obj.get_uri(session)
            retobj = fibrechannel_switch()
